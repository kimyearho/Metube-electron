...
<template>
  ...
  <div>
    ...
    <div v-if="isLogin">
      ...
      <draggable tag="md-list"
        ...
        <md-list-item v-for="(item, index) in playlist"
          ...
          <span class="md-list-item-text music-title cursor"
                ...
                @click="playItem(index)">{{ item.title }}</span>
          <span class="label_video"
                v-if="item.videoId && item.isLive != 'live'">{{ item.duration }}</span>
          <span class="label_live"
                v-if="item.videoId && item.isLive == 'live'">LIVE</span>
          <a class="cursor"
      ...
      <my-context-menu :index="selectedContextIndex"
                       :data="selectedData"
                       :isShow="contextShow"
    ...
    <div v-else>
      <el-row>
        <el-col>
          <el-card style="height: 200px; margin-top: 70px; margin-left: 30px; margin-right: 30px;"
                   class="thumb"
                   :body-style="{ padding: '0px' }">
            <img class="md-image thumbnail"
          ...
          <div style="text-align: center; margin-top: 20px;">
            <md-button class="cursor md-raised md-primary"
                       style="width: 120px;"
                       @click="signLink">{{ $t('SIGN.SIGN_IN') }}</md-button>
          </div>
        </el-col>
      </el-row>
    </div>
    ...
    methods: {
      ...
      reloadCollection() {
        this.$modal.show('dialog', {
          title: 'Success',
          text: this.$t('CONTEXT.MESSAGE.COVER_CHANGE_SUCCESS'),
          buttons: [
            {
              title: 'Close'
            }
      ...
      feachExtends() {
        ...
        if (musicInfo) {
          ...
          if (musicInfo.name !== this.id) {
            ...
            if (Number(musicInfo.index) === Number(this.startIndex)) {
              ...
              this.videoActive(Number(musicInfo.index))
            } else {
              // 인덱스가 서로 다르므로 새로 시작
              this.autoStart()
            }
      ...
      autoStart() {
        ...
        // 뒤로가기로 접근시 undefined가 발생하므로 예외처리한다
        if (this.startIndex !== undefined) {
          playingItem.index = Number(this.startIndex)
      ...
      playSetting(playingItem, event) {
        ...
        this.cover = playingItem.imageInfo ? playingItem.imageInfo : playingItem.thumbnails
      ...
      nextTrackScroll(duration) {
        if (this.$route.name === 'MY-PLAYING-PLAYLIST') {
          const cancelScroll = this.$scrollTo(`#item${this.selectedIndex}`, duration, options)
          setTimeout(() => {
            cancelScroll()
          }, 1500)
        }
