  ...
  <div>
    ...
    <div class="wrapper">
        ...
        <el-col>
          <div class="menu1_tip">
            <div>
              ...
              <md-button class="md-raised b-primary c-add md-primary"
                         @click="create"
                         v-if="playType !== 'play' && playType !== 'channel'">
      ...
      <el-row class="el-scroll" :class="{ dynamicHeight: isSub }">
          ...
          <div class="menu1">
            <label class="wh" v-if="playType === 'play'">
                <strong style="margin-left: 5px;">
                  {{ $t('COLLECTION.MENU.PLAY_LIST') }}
                  <span
                    style="float:right;"
                  >({{ playlists.length }}/50)</span>
                </strong>
              </label>
            <label class="wh" v-else-if="playType === 'channel'">
                <strong style="margin-left: 5px;">
                  {{ $t('COLLECTION.MENU.CHANNEL') }}
                  <span
                    style="float:right;"
                  >({{ playlists.length }}/50)</span>
                </strong>
            ...
            <label class="wh" v-else>
                <strong style="margin-left: 5px;">
                  {{ $t('COLLECTION.MENU.MY_COLLECTION') }}
                  <span
                    style="float:right;"
                  >({{ playlists.length }}/20)</span>
                </strong>
              </label>
          </div>
        </el-col>
        ...
        <!-- NO PLAYLIST -->
        <el-col class="noList"
                v-if="isLogin && playlists.length === 0">{{ $t('COLLECTION.NO_PLAYLIST') }}</el-col>
        ...
        <el-col v-else class="cols" v-for="item in playlists" :key="item._id" :span="12">
          <el-card class="thumb" :body-style="{ padding: '0px' }">
            <div class="overlay">
              ...
              <div class="playWrapper cursor"
                   @click="showMusicList(item)"
                   :class="{ channelWrapper: playType === 'channel' }">
                <div class="overlayMenu" :class="{ channelMenu: playType === 'channel' }"></div>
              </div>
            </div>
