    ...
    <div class="zaudio_player">
      <div class="zaudio_playercontrols">
        ...
        <div class="zaudio_seekbar">
          ...
          <progress class="cursor"
                    @click="seekTo"
                    :value="range"
                    min="0"
                    :max="maxTime" />
          <span class="playingLive"
                v-if="isLive">L I V E - P L A Y I N G</span>
        ...
        <div class="zaudio_buttonwrapper"
          ...
          <div class="zaudio_playercontrolbuttons">
            ...
            <img class="cursor md-image" v-else
                 width="35"
                 height="15"
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      // 다음 음악재생 이벤트 수신
      this.$eventBus.$on('sendNextMusicPlay', this.sendNextMusicPlay)

      // 다음 페이지 이벤트 수신
      this.$eventBus.$on('sendNextPage', this.sendNextPage)
    },
    methods: {
      // 다음 재생 이벤트 전달
    ...
    beforeDestroy() {
      ...
      this.$eventBus.$off('playVideoSecond')
      this.$eventBus.$off('sendNextMusicPlay')
      this.$eventBus.$off('sendNextPage')
    }
  }
