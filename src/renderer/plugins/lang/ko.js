export default {
  MAIN: {
    APP_NAME: "M e t u b e",
    HISTORY: {
      INDEX: "재생된 비디오 목록",
      RECENT: "최근 검색어"
    },
    MENU: {
      SEARCH: "검색",
      COLLECTION: "컬렉션",
      HISTORY: "기록",
      SIGN: "로그인",
      SETTING: "설정"
    }
  },
  COMMONS: {
    LABEL: {
      CHANNEL: "채널",
      PLAY_LIST: "재생목록",
      LIVE: "실시간",
      RELATED: "연관목록"
    },
    DIALOG: {
      ERROR_URL: "입력하신 URL은 유효하지 않습니다.",
      BLANK_FORM: "URL을 입력하세요."
    },
    COLLECTION: {
      REGISTER: "컬렉션으로 등록하시겠습니까?",
      REMOVE: "컬렉션에서 제외하시겠습니까?"
    },
    NO_KEYWORD: "최근에 검색 한 키워드가 없습니다",
    MORE: "다음 페이지",
    END: "페이지 끝"
  },
  PLAYERBAR: {
    JUMP: "재생 위치로이동",
    REPEAT_OFF: "반복",
    REPEAT_ON: "반복",
    VOLUME_OFF: "음량",
    VOLUME_ON: "음량",
    BUTTON_DELAY: "😅 3초후 사용할 수 있습니다."
  },
  SETTING: {
    PLAYER_HIDE:
      "플레이어 창을 숨깁니다. 그러나 앱을 종료하면 설정이 초기화 됩니다.",
    ALWAYS_TOP: "미튜브 윈도우를 항상 위에 놓습니다.",
    LOCALE: "클라이언트 언어를 설정합니다.",
    RELEASE: "현재 클라이언트는 최신 버전 입니다.",
    NEW_RELEASE:
      "새 버전이 출시되었습니다. 다운로드 하시겠습니까? (이 메시지는 지금 만 표시되며 앱을 다시 시작하면 표시됩니다.)",
    MENU: {
      PLAYER_HIDE: "플레이어 숨김",
      ALWAYS_TOP: "항상 위",
      LOCALE: "언어",
      VERSION: "버전",
      RELEASE: "배포"
    }
  },
  SIGN: {
    SIGN_IN: "구글 로그인",
    SIGN_OUT: "로그아웃",
    NOTICE: "공지",
    NO_LOGIN_NOTICE:
      "미튜브에 로그인하면 미튜브에서 감상한 비디오가 유튜브와 동기화되며, 미튜브에서 컬렉션을 관리할 수 있습니다.",
    YES_LOGIN_NOTICE:
      "미튜브에 로그인 되었습니다. 로그인 사용자는 컬렉션 및 부가기능을 이용하실 수 있습니다."
  },
  COLLECTION: {
    INDEX: "각 항목에 대해 최근 4개 컬렉션을 표시 합니다.",
    ALBUM_INDEX: "모든 재생목록을 조회합니다.",
    NO_LOGIN: "로그인 후 이용하실 수 있습니다.",
    NO_LOGIN_BUTTON_LINK: "로그인으로 이동",
    NO_PLAYLIST: "등록된 재생목록이 없습니다. 재생목록을 등록하세요.",
    NO_CHANNEL: "등록된 채널이 없습니다. 채널을 등록하세요.",
    REMOVE_ALBUM: "이 컬렉션을 삭제 하시겠습니까?",
    REMOVE_VIDEO: "선택한 비디오를 삭제 하시겠습니까?",
    REMOVE_NOT_ALBUM: "비디오가 현재 컬렉션에서 재생 중이므로 이 컬렉션을 삭제할 수 없습니다.",
    REMOVE_ALBUM_SUCCESS: "컬렉션이 삭제 되었습니다. 버튼을 누르면 목록으로 돌아갑니다.",
    CONTINUE: "음악을 계속 재생하려면 로그인하십시오.",
    COLLECTION_MAX_TOTAL_OVER: "생성할 수 있는 최대 개수를 초과하여 더이상 생성할 수 없습니다. 최대 개수는 50개 입니다.",
    MYCOLLECTION_MAX_TOTAL_OVER: "생성할 수 있는 최대 개수를 초과하여 더이상 생성할 수 없습니다. 최대 개수는 20개 입니다.",
    MYCOLLECTION_MUSICSET_TOTAL_OVER: "더 이상 추가할 수 없습니다. 추가할 수 있는 최대개수는 100개 입니다.",
    MENU: {
      COLLECTION: "컬렉션",
      MY_COLLECTION: "내 컬렉션",
      PLAY_LIST: "재생 목록",
      CHANNEL: "채널"
    },
    ADD_VIDEO_LABEL: "추가",
    CREATE_COLLECTION_LABEL: "생성"
  },
  CONTEXT: {
    MESSAGE: {
      CLIPBOARD_SAVE: "😁 링크가 클립보드에 저장되었습니다.",
      CLIPBOARD_FAIL: "😥 링크를 클립보드에 저장에 실패했습니다.",
      VIDEO_REMOVE_FAIL: "😥 재생중인 비디오는 삭제할 수 없습니다.",
      COVER_CHANGE: "이 비디오의 썸네일 이미지로 내 컬렉션 커버를 변경하시겠습니까?",
      COVER_CHANGE_SUCCESS: "😁 이 컬렉션의 커버 이미지가 변경 되었습니다. (재생중일때는 표시 되지 않습니다)",
      ADD_COLLECTION_SUCCESS: "😁 선택한 컬렉션에 등록 되었습니다.",
      ADD_COLLECTION_FAIL: "컬렉션 등록 중 오류가 발생했습니다. 재시도 하거나, 개발자에게 이슈를 알려주세요."
    }
  },
  HISTORY: {
    NO_LOGIN: "로그인 후 이용하실 수 있습니다.",
    MENU_NAME: "재생 기록",
    MENU_DESC: "최근에 재생했던 20개의 비디오를 표시합니다."
  }
};
